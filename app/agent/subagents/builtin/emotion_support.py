...
NEGATIVE_EMOTION_PATTERNS = [
    ...
    re.compile(r"(焦虑|难受|低落|抑郁|沮丧|崩溃|压力大|烦躁)", re.IGNORECASE),
]
...
class EmotionSupportSubagent(BaseSubagent):
    ...
    def _build_non_budget_toolset(self, tool_names: list[str]) -> list[str]:
        return [
            ...
            if not self._is_budget_tool(tool_name)
        ]
    ...
    def _should_offer_budget_adjustment(self, text: str) -> bool:
        ...
        has_overeat = any(pattern.search(text) for pattern in OVEREAT_PATTERNS)
        return has_negative and has_overeat
    ...
    def _build_ui_action_background_hint(self, action_payload: dict) -> str:
        ...
        capability_line = (
            "本轮已向前端下发预算调整交互卡片（10 秒后默认 +100 kcal 自动执行），"
            "请你不要再次调用预算调整工具。"
            if can_apply
        ...
        return (
            ...
            f"- {capability_line}\n"
            f"- {snapshot_line}\n"
            "- 回复结构：先共情 → 给出 2 条小步行动 → 提供 2~3 条放松场景建议。\n"

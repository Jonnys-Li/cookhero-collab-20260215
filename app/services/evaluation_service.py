...
class FilteredChatOpenAI:
    ...
    def _filter_kwargs(self, kwargs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {k: v for k, v in kwargs.items() if k not in self.UNSUPPORTED_PARAMS}
...
class EvaluationService:
    ...
    def _init_ragas(self):
        ...
        try:
            ...
            filtered_llm = FilteredChatOpenAI(base_llm)
                ...
                if score_list and len(score_list) > 0:
                    ...
                    for metric_name in self.config.metrics:
                        if metric_name in first_score:
                            ...
                            scores[metric_name] = float(value) if value is not None and not math.isnan(value) else None
                    ...
                    try:
                        ...
                        if hasattr(result, metric_name):
                            ...
                            if hasattr(value, '__iter__') and not isinstance(value, str):
                                scores[metric_name] = float(list(value)[0]) if value else None
                            else:
                                scores[metric_name] = float(value) if value is not None and not math.isnan(value) else None
                    except Exception:
                        pass
    ...
    async def schedule_evaluation(
        ...
        try:
            ...
            if self.config.async_mode:
                asyncio.create_task(
                    self._run_evaluation(str(evaluation.id), query, context, response)
                )
            else:
                await self._run_evaluation(str(evaluation.id), query, context, response)

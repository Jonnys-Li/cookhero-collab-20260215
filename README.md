...
- **内置工具**：计算器、日期时间、文本处理等实用工具
- **可扩展架构**：支持自定义 Agent 和 Tool 注册
- **独立会话管理**：Agent 会话与标准对话分离
...
- **添加新 Agent**：继承 `BaseAgent` 并使用 `@register_agent` 装饰器
- **添加新 Tool**：继承 `BaseTool` 并使用 `@register_tool` 装饰器
